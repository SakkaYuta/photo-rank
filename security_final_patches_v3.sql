-- PhotoRank v5.0 最終セキュリティ再点検 - 最終確実版
-- 全ての既存チェックを完全にして重複エラーを回避
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        AND indexname = 'idx_purchases_pi_unique'
    ) THEN
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        AND constraint_name = 'webhook_events_event_id_unique'
...
SELECT 'Security Patches Applied Successfully (V3 - Final)' as status, now() as completed_at;