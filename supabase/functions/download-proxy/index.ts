...
serve(async (req) => {
  ...
  try {
    ...
    if (pathParts.length < 2) {
      return new Response('Invalid file path format', { status: 400, headers: corsHeaders })
    }
    ...
    if (!authorized) {
      return new Response('Access denied to this file', { status: 403, headers: corsHeaders })
    }
