...
-- =====================================================================
-- 作成日: 2025-10-02
-- 目的: v6完全スキーマ適用（外部キー制約、fulfillments、推奨方針B準拠）
-- 適用方法: Supabase Studio SQL Editor から実行
-- 特徴: 安全なFK追加、NULL値排除、実データ導出
-- =====================================================================
...
-- =============================================================================
-- 6. fulfillments テーブル（manufacturing_partner_id FK 安全追加）
-- =============================================================================
...
SELECT
  ...
  -- Product info (products由来 - 顧客向け名称)
  pr.id AS product_id,
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '═══════════════════════════════════════════════════════════════';
  RAISE NOTICE '✅ v6 完全スキーマSQL適用完了（最終版）';
  RAISE NOTICE '═══════════════════════════════════════════════════════════════';
  RAISE NOTICE '';
  ...
  RAISE NOTICE '  ✅ fulfillments (manufacturing_partner_id FK: ON DELETE SET NULL)';
  RAISE NOTICE '  ✅ creator_organizers';
  RAISE NOTICE '  ✅ refunds';
  RAISE NOTICE '';
  RAISE NOTICE '外部キー制約:';
  RAISE NOTICE '  ✅ fulfillments.manufacturing_partner_id → manufacturing_partners(id) [SET NULL]';
  RAISE NOTICE '  ℹ️  データ整合性保護';
  RAISE NOTICE '';
  RAISE NOTICE '作成されたビュー:';
  RAISE NOTICE '  ✅ sales_vw (v5 sales + purchases)';
  RAISE NOTICE '  ✅ factory_orders_vw (v6 fulfillments - 推奨方針B)';
  RAISE NOTICE '      - 顧客向け商品名（products.title）';
  RAISE NOTICE '      - NULL値なし、実データ導出';
  RAISE NOTICE '      - 注文詳細・顧客情報含む';
  RAISE NOTICE '  ✅ publishing_approvals_vw (v5 works)';
  RAISE NOTICE '  ✅ purchases_vw (v5 purchases)';
  RAISE NOTICE '  ✅ works_vw (v5 works)';
  RAISE NOTICE '  ✅ users_vw (v5 users + profiles)';
  RAISE NOTICE '  ✅ refund_requests_vw (v5 refund_requests)';
  RAISE NOTICE '';
  ...
  RAISE NOTICE '  SELECT product_name, customer_name FROM factory_orders_vw LIMIT 5;';
  RAISE NOTICE '';
  RAISE NOTICE '✅ v6完全スキーマ適用完了！';
  RAISE NOTICE '';
  RAISE NOTICE '推奨運用:';
  RAISE NOTICE '  - データ整合性: FK制約により自動保護';
  RAISE NOTICE '  - 将来拡張: partner_products テーブル追加可能（方針C）';
  RAISE NOTICE '';
  RAISE NOTICE '═══════════════════════════════════════════════════════════════';
END $$;
