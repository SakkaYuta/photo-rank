-- PhotoRank v5.0 最終セキュリティ再点検 - 完全修正版
-- エラー解決：関数定義を先に実行、実際のテーブル構造に完全対応

-- =====================================
-- 1. 必要な関数の事前定義（エラー回避）
-- =====================================

-- is_admin_strict 関数の作成（最優先）
...
-- =====================================
-- 2. purchases と Stripe Webhook の整合性 (CRITICAL)
-- =====================================
...
BEGIN
    ...
    ELSE
        -- 適切なカラムが見つからない場合は buyer_user_id を追加
        ALTER TABLE public.purchases ADD COLUMN buyer_user_id uuid;
        user_col := 'buyer_user_id';
        RAISE NOTICE 'Added buyer_user_id column to purchases table';
    END IF;
    ...
    -- 動的にポリシーを作成（関数が確実に存在する状態で実行）
    sql_text := format('
        CREATE POLICY purchases_buyer_access ON public.purchases 
        FOR SELECT USING (
            %I = auth.uid() OR 
...
-- =====================================
-- 4. webhook_events の一意化とRLS（完全安全版）
-- =====================================
...
-- =====================================
-- 5. manufacturing_orders の updated_at 補完（安全版）
-- =====================================
...
-- 最終確認メッセージ
SELECT 
  'Security Patches Applied Successfully (Fixed Version)' as status,
  now() as applied_at,
  'Functions created first, then policies applied safely' as details;