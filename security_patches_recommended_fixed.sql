-- PhotoRank v5.0 中優先度セキュリティパッチ（修正版）
-- 実行順序: security_patches_critical_fixed.sql の後に実行してください
...
-- =====================================
-- 2. payouts/sales拡張RLS（修正版）
-- =====================================

-- payouts_v31 の組織メンバーアクセス拡張（実際のカラム名に修正）
-- 注意: 実際のテーブル構造に合わせて user_id カラムを使用
...
FOR SELECT USING (
  ...
  EXISTS (
    SELECT 1 FROM public.organizer_members om
    JOIN public.works w ON w.user_id = payouts_v31.user_id  -- 実際のカラム名に修正
...
-- sales の組織メンバーアクセス拡張（実際のカラム名に修正）
DROP POLICY IF EXISTS "sales_extended_access" ON public.sales;
CREATE POLICY "sales_extended_access" ON public.sales
FOR SELECT USING (
  user_id = auth.uid() OR  -- sales テーブルの実際のカラム名に修正
  public.is_admin_user(auth.uid()) OR
  -- 組織メンバーもアクセス可能（将来拡張）
  EXISTS (
    SELECT 1 FROM public.organizer_members om
    JOIN public.works w ON w.user_id = sales.user_id  -- 実際のカラム名に修正
    WHERE om.user_id = auth.uid() 
    AND om.is_active = true
    AND om.role IN ('admin', 'member')
  )
...
BEGIN
  ...
  -- Webhook Events テーブル存在確認
  RETURN QUERY
  SELECT 
    'webhook_events_table'::text,
    CASE WHEN EXISTS (
      SELECT 1 FROM information_schema.tables 
      WHERE table_schema = 'public' AND table_name = 'webhook_events'
    ) THEN 'OK' ELSE 'ERROR' END::text,
    'Webhook events table existence check'::text;
...
-- 推奨セキュリティパッチの適用確認（修正版）
SELECT 
  'Recommended Security Patches Applied (Fixed Version)' as status,
  now() as applied_at,
  (SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'organizer_members') as organizer_members_table_ready,
  (SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'service_configs') as service_configs_ready;