...
export class PurchaseService {
  ...
  // 一括購入
  async initiateBulkPurchase(workIds: string[]): Promise<BulkPurchaseFlowResult> {
    try {
      const { data: { user } } = await supabase.auth.getUser()
      if (!user) return { status: 'failed', error: 'ログインが必要です' }
      ...
      try {
        // 一括決済Intent作成
        const { clientSecret } = await this.createBulkPaymentIntent(successfulLocks)
        if (!clientSecret) {
          // ロック解除
  ...
  // 一括決済Intent作成（内部メソッド）
  private async createBulkPaymentIntent(workIds: string[]) {
    const { data, error } = await supabase.functions.invoke('create-bulk-payment-intent', {
      body: { workIds, userId: (await supabase.auth.getUser()).data.user?.id },
    })
