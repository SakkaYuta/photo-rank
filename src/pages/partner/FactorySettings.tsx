...
const FactorySettings: React.FC = () => {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <h1 className="text-2xl font-bold mb-4">工場設定</h1>

      <div className="bg-white rounded-lg shadow p-6 space-y-4">
        <div>
          <label className="block text-sm text-gray-600 mb-1">工場名</label>
          <input className="input input-bordered w-full" value={partner?.name || 'デモ製造パートナー'} readOnly />
        </div>
        <div>
          <label className="block text-sm text-gray-600 mb-1">通知メール</label>
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">注文作成/更新時に通知します（Stripe決済/製造連携用）</p>
        </div>
        <div>
          <label className="block text-sm text-gray-600 mb-2">有効な商品種別</label>
          <div className="flex flex-wrap gap-2">
            {products.map(p => (
              <span key={p.id} className="badge badge-outline">{p.product_type}</span>
            ))}
            {products.length === 0 && <span className="text-sm text-gray-500">商品が見つかりません</span>}
          </div>
        </div>
